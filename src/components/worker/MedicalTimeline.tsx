...
export const MedicalTimeline: React.FC<MedicalTimelineProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Top Action Bar: Filters + Camera Scan Action Button */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {onOpenScanner && (
          <button
            ...
            className="btn-minimal-primary bg-sky-600 hover:bg-sky-700 text-white flex items-center justify-center gap-2 py-2 px-3.5 text-xs font-semibold shadow-xs cursor-pointer shrink-0 border-sky-600 hover:border-sky-700"
          >
      ...
      {filteredRecords.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-slate-200 p-6 space-y-4 shadow-[0_1px_2px_0_rgba(15,23,42,0.03)]">
          <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center mx-auto text-slate-400 border border-slate-200">
          ...
          {onOpenScanner && (
            <button
              type="button"
              onClick={onOpenScanner}
              className="btn-minimal-primary text-xs cursor-pointer inline-flex items-center gap-2"
            >
              <Camera className="w-3.5 h-3.5" />
                  ...
                  <div className="p-3 rounded-lg bg-slate-50 border border-slate-200/90 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-sky-700 hover:text-sky-900 text-xs font-semibold flex items-center gap-1 cursor-pointer"
                      >
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto py-1">
                      {record.attachments!.map((imgUrl, idx) => (
                        <button
                          ...
                          className="group relative w-16 h-20 rounded-lg overflow-hidden border border-slate-200 hover:border-sky-500 shadow-xs transition cursor-pointer shrink-0 bg-white"
                        >
                          <img
                            src={imgUrl}
